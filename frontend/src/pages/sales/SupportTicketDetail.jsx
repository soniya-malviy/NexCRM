...
export default function SupportTicketDetail() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <button onClick={() => navigate('/sales/support')} className="flex items-center gap-2 text-sm text-gray-400 hover:text-white transition-colors">
        <ArrowLeft size={16} /> Back to Support Requests
      </button>

      <div className="glass-panel p-8 rounded-xl border border-white/10 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/10 blur-[80px] rounded-full pointer-events-none -mr-20 -mt-20"></div>
        ...
        <div className="flex items-start justify-between mb-8 relative z-10">
          ...
          <div>
            <span className={`font-bold uppercase tracking-wider text-xs px-4 py-2 rounded-lg border ${statusColors[ticket.status]}`}>
              {ticket.status}
            </span>
          </div>
        </div>
          ...
          <div className="space-y-6">
            <div className="bg-dark-surface/50 border border-white/5 p-5 rounded-xl">
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500/20 to-purple-500/20 border border-blue-500/30 flex items-center justify-center text-blue-400 font-bold">
    ...
    <div className="mt-8 border-t border-white/10 pt-8">
      ...
      <div className="space-y-4 mb-6 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
        {(!messages || messages.length === 0) && (
          <p className="text-sm text-gray-500 italic text-center py-4">No messages yet. Reply below to start the conversation with Support.</p>
        )}
        {messages?.map(msg => (
      ...
      <form onSubmit={handleSend} className="relative">
        <textarea 
          ...
          placeholder="Ask for help or provide more details..."
          className="w-full bg-dark-bg border border-white/10 rounded-xl pl-4 pr-24 py-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 min-h-[100px] resize-y"
        />
        <button 
          type="submit" 
          disabled={sending || !content.trim()}
          className="absolute bottom-3 right-3 px-4 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium rounded-lg transition-colors disabled:opacity-50"
        >
